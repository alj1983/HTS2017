...
[fn:cow] In some modern file systems that make use of Copy-On-Write,
there won't actually be any copying of the data unless one of the
files is modified.

[fn:vcs] How to handle lots of files created by various versions of
...
ask the shell (in this case bash[fn:shells]) to run the commands
...
[fn:shells] bash stands for Bourne Again Shell, and is a bit of a joke
...
always there are people who consider it an abomination.
