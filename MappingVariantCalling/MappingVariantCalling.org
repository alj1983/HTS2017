...
Ion Torrent data (see further below for Illumina paired-end data):


#+begin_src sh
mkdir Mapping

cp GenomeAssembly/IonTorrenteNovoAssembly_assembly/IonTorrentDeNovoAssembly_d_results/ \
...
=RAWREADS_val_1.fq= and 
=RAWREADS_val_2.fq=, and the index for our assembly genome is in 
=GENOMENAME.X.bt2=, the command without any of the optional options would be:

#+begin_src sh
bowtie2 -x GENOMENAME -1 RAWREADS_val_1.fq \
-2 RAWREADS_val_2.fq -S MAPPED.sam
#+end_src
